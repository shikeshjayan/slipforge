...
const required = (msg) => (v) =>
  v !== undefined && v !== null && String(v).trim() ? undefined : msg
...
const DataEntry = () => {
  ...
  const [items, setItems] = useState(
    draft?.items?.length ? draft.items : [{ ...defaultItem }],
  );
  ...
  }, [settings.businessName, settings.taxRate, settings.currency])
            ...
            <div className="space-y-3">
              {items.map((item, i) => (
                <div
                  ...
                  <div className="text-xs font-medium text-slate-400 dark:text-gray-500 mb-2">
                    Item {i + 1}
                  </div>
                  <div className="space-y-3">
                    <div>
                      <label
                        htmlFor={`desc-${i}`}
                        className="block text-xs font-medium text-slate-600 dark:text-gray-400 mb-1">
                        Description
                      </label>
                      <input
                        type="text"
